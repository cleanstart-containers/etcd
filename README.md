...
**Image Path**: `ghcr.io/cleanstart-containers/etcd`
...
```bash
docker pull ghcr.io/cleanstart-containers/etcd:latest
```
```bash
docker pull ghcr.io/cleanstart-containers/etcd:latest-dev
```
...
```bash
docker run -it --name etcd ghcr.io/cleanstart-containers/etcd:latest
```
...
docker run -d --name etcd-prod \
  ...
  ghcr.io/cleanstart-containers/etcd:latest
```

Volume Mount Mount local directory for persistent data

```bash
docker run -v /app:/app ghcr.io/cleanstart-containers/etcd:latest
```

Port Forwarding Run with custom port mappings

```bash
docker run -p 8080:8080 ghcr.io/cleanstart-containers/etcd:latest
```
